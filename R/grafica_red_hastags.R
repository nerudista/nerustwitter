...
grafica_hashtags <- function(user,corp_tmln, folder){
  ...
  quanteda::textplot_network(topgat_fcm, min_freq = 0.1, edge_alpha = 0.7, edge_size = 4) %>%
    ggplot2::ggsave(filename = paste0( folder,"/red_hastags_",user,".png"),
           device = "png",
           dpi = 72)
